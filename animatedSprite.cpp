...
AnimatedSprite::AnimatedSprite(Graphics &graphics, const std::string &filePath, int sourceX, int sourceY,
	int width, int height, float posX, float posY, float timeToUpdate) :
		...
		_frameIndex(0), _timeToUpdate(timeToUpdate), _visible(true), _currentAnimationOnce(false), _currentAnimation(""), _timeElapsed(0)
{}
...
void AnimatedSprite::draw(Graphics &graphics, int x, int y) {
	if (this->_visible) {
		...
		destRect.h = this->_sourceRect.h * global::SPRITE_RATIO;
		destRect.w = this->_sourceRect.w * global::SPRITE_RATIO;
